...
\begin{document}
	...
	\\
	Before we do that, however, let us see what goes wrong when using our current definition of manifolds. Recall that our overall goal is to develop a theory so we may discuss curvature. Curvature, we understand, is a bending or change of direction of an object (or in our case, manifolds). However, it's not just any change of direction or bending. For example, a zig-zag line changes direction frequently, however, one wouldn't consider it ``curving'' through space. For something to be curved, it requires a smooth change of direction. \\
	\begin{example}
		Both follow a very similar path through space. However, the image on the left consists of sharp and jagged turns while the one on the right takes smoother turns. The one on the left does not appear to curve while the one on the right does.
		\begin{center}
			\begin{displayfigure}[h]
	...
	Up to this point, we have really only been developing the preliminary topology and definitions necessary for any coherent discussion of curvature. We have done little in the way of discussing the core focus of this paper \textit{curvature.} That changes now. In this section and those that follow, we will finally utilize our new language and tools from calculus and linear algebra to discuss the nature of curvature. To do this, we will first construct the \textit{tangent space} to a surface. This idea is fundamental to the rest of the paper, so many examples and visuals will be given to aid the reader's understanding of it. However, the rigorous construction of the tangent space will be left to those who wish to take a course in differential geometry. The purpose here is to give the intuition and insight to this concept. Once done, we will introduce the notion of a metric on a space. For those familiar with analysis, this notion of a metric is different from that learned when studying metric spaces, however they are very related.  
	
	\subsection*{The Tangent Space}
	You, the budding mathematician eager to apply these new ideas involving manifolds have decided to take it upon yourself to study a number of manifolds. You grab a trusty atlas so you can navigate around it successfully. As you walk along the manifold, using the atlas to keep track of where you are at, you feel excitement as you explore this newfound world of manifolds. Keen to learn more, you make observations. You note that 
